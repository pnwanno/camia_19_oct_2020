...
class Conferences extends StatefulWidget{
  _Conferences createState(){
    return _Conferences();
  }
}

class _Conferences extends State<Conferences>{
  ...
  fetchLocal()async{
    ...
    List _result= await _con.rawQuery("select * from cm_news where category='Conferences' order by cast(news_date_str as signed) desc");
    ...
    if(_isFetching==false){
      ...
      try{
        http.Response _resp= await http.post(
          globals.globBaseNewsAPI + "?process_as=fetch_conferences",
          body: {
  ...
  populateDB(String _dataStr)async{
    ...
    String _newsCategory="Conferences";
    for(int _k=0; _k<_kount; _k++){
